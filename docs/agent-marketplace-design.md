...
> Working design doc for the Agent Marketplace (issue #200, PR #287). Captures the decisions coming out of the review round and a design meeting, grounded against the code on `feat/200-agent-marketplace-api`. Not wired into the website sidebar — this is an internal ADR-style record, not user-facing product docs. Two items marked **NEEDS SIGN-OFF** are genuinely @robert-cronin's call.
...
- Binding: `AgentDeployment.spec.models[]` (`MaxItems=1` after review) with exactly one of three modes per binding — `deploymentRef`, `gatewayEndpoint`, `externalAPI` (`agentdeployment_types.go:37-185`). `gatewayEndpoint` already carries a served `modelName` and is the OpenAI-native / BBR-routed path (`docs/gateway.md:195`). `ModelBinding.Name` is a **slot key** used by `spec.config` to reference the resolved binding — it is *not* the model's name, which is why `name: default` next to `modelName` reads as a smell.
...
### 1. Model binding — converge on the OpenAI-native gateway path  **NEEDS SIGN-OFF (removal timing)**
...
**Sign-off needed:** whether/when to *deprecate or remove* `deploymentRef` entirely vs. keep it as sugar indefinitely. Recommendation is keep-as-sugar; removal is a product call.
...
### 5. `providerOverrides` escape hatch for security context  **NEEDS SIGN-OFF (security surface)**
...
**Sign-off needed:** confirm that a user-facing security-context override (even validated + limited-key) is acceptable, given [8] just narrowed the surface. If not, we keep security entirely provider-owned and scope `providerOverrides` to non-security fields only.
...
|---|------|-----------|--------|
| T1 | Catalog → annotation on `AgentProviderConfig` (parser + helpers + optional webhook validation + manifests regen) | — | ready |
| T2 | Resolve `deploymentRef` via `servedName` + gateway endpoint (lower it to the `gatewayEndpoint` path; direct-Service fallback when no gateway) | — | ready |
| T3 | Collapse `spec.models[]` → single `spec.model`; drop the `name: default` slot key; update config references, tests, demo manifests | decision 1 sign-off | blocked |
| T4 | Extract agent providers to `providers/agent-*` own-module dirs behind a shared provider interface (kagent, orka, container) | — (do after T1/T2 to avoid churn) | ready |
| T5 | Install-instructions annotation + `OperatorNotInstalled` condition surfaced to UI | T4 | ready-after-T4 |
| T6 | Add `spec.provider.overrides` `RawExtension` + webhook allow-list validation for security-context overrides | decision 5 sign-off | blocked |
| T7 | Docs: update `docs/crd-reference.md` + `docs/gateway.md` for the binding convergence and catalog move | T1–T3 | ready-after |

## Out of scope / follow-ups

- PR #287 review comment [12] (full per-provider anthropic/azure `apiType` mapping) — separate follow-up; `apiType` is already preserved in `status.modelBindings`.
